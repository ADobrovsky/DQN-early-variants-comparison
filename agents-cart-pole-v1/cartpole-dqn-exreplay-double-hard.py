...
def optimize_model():
    ...
    next_state_values_target = torch.zeros((BATCH_SIZE, 2), device=device)
    with torch.no_grad():
    ...
    wandb.log({"loss ": loss, "mean_state_action_value": torch.mean(state_action_values),
               "mean_next_state_values": torch.mean(next_state_values_target),
...
TARGET_UPDATE = 50  # update rate for double deep Q-learning with hard update
...
MEMORY_SIZE = 10000
TYPE = 'Double-DQN-hard-update-dueling'
LR_NUM = "4"
...
for i_episode in range(num_episodes):
    ...
    for t in count():
        ...
        if i_episode % TARGET_UPDATE == 0:  # update target network every i episodes
        ...
        if done:
            ...
            if len(durations_t) >= 100:
                ...
                #  therefore, check the last 100 durations
                #for i in range(len(episode_durations) - 100, len(episode_durations) - 1):
                 #   mean += episode_durations[i]
                # mean value of the last 10 episodes
                #mean = mean / 100

                #  checkpoint save of the network for the best 100 episode average duration
                if (means[i_episode].item() > highest_mean_duration):
                    highest_mean_duration = means[i_episode].item()
