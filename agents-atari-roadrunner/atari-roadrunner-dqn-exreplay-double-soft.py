...
def optimize_model():
    ...
    # on the policy_net; selecting their best  action with max(1)[1]
    # based on the selected states of policy_net, the expected values
    # are computed by the target networ.
    # All is merged based on the mask, such that we'll have either the expected
    # state value or 0 in case the state was final.
    next_state_values_policy = torch.zeros((BATCH_SIZE, 1), device=device, dtype=torch.long)  # , dtype=torch.long
    next_state_values_target = torch.zeros((BATCH_SIZE, n_actions), device=device)
    with torch.no_grad():

        # return next best actions of policy net
        next_state_values_policy[non_final_mask] = policy_net(non_final_next_states).max(1)[1].unsqueeze(
            1)  # we need the indices of the actions returned by max
        # print("next state value policy: \n" + str(next_state_values_policy))

        next_state_values_target[non_final_mask] = target_net(non_final_next_states)
        #  print("next state value target: \n" + str(next_state_values_target))
        next_state_values_target = next_state_values_target.gather(1, next_state_values_policy)
        #  print("next state value gathered: \n" + str(next_state_values_target))

        next_state_values = next_state_values_target.squeeze(1)
    ...
    wandb.log({"loss ": loss.item(), "mean_state_action_value": torch.mean(state_action_values).item(),
               "mean_next_state_values": torch.mean(next_state_values).item(),
               "mean_expected_state_action_values": torch.mean(expected_state_action_values).item(),
               "mean_reward_batch": torch.mean(reward_batch, dtype=torch.float).item(), "episode": i_episode})
...
# REPLAY MEMORY
memory = ReplayMemory(50000)  # buffer for replay memory, 10000 #TODO larger necessary
...
MEMORY_SIZE = 50000
TYPE = 'atari-DQN-double-soft-update-dueling'
LR_NUM = "1-4-a"
...
for i_episode in range(num_episodes):
    ...
    for t in count():
        ...
        # apply action 4 times for 4 frames ? or is this automatically done by frameskip?
        ...
        if done:
            ...
            # free some ressources to avoid CUDA out of memory
            gc.collect()
